...
class Boy:
    ...
    def fire_ball(self):
        if self.ball_count > 0:
            self.ball_count -= 1
            fireball = Ball(self.x, self.y, self.face_dir*10)
            game_world.add_object(fireball)
            # 발사된 공 - 좀비
            game_world.add_collision_pair('fireball-zombie', fireball, None)

    def get_bb(self):
        # 4개의 값을 리턴하는데, 사실은 1개의 튜플 리턴
